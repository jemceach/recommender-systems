...
We found that the more memory intense approach, user-based collaborative filtering, faired better in terms of error metrics as user-based had lower RMSE at .356 compared to the the item-based at .447. This could be due to the fact that the users are very similar as they are all Mexican college students and so it would make send that the user collaborastive filtering provides more accuraate results. 
...
After running the following algorithms :
- Item-based - center normalized
- User-based - center normalized
- SVD - center normalized
- SVD - Z-score normalized
- SVD funk

We can conclude that the SVD algorithm using a normalized center showed the best results overall with an RMSE of ~ 0.3699. This means the SVD algo would provide the most relevant restaurant recommendations to Mexican college students. 
However once we calculated the diversity score, we now see that FunkSVD performed the best with a score of .0184, so this recommender would give the most diverse restaurant recommendations to these college students. 
