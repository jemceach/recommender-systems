...
evalu <- evaluationScheme(umat, method="split", train=0.9, given=4, goodRating=0)
```
...
```{r}
data %>% select(Rcuisine) %>% group_by(Rcuisine) %>% add_tally() %>% ggplot(aes(x=reorder(Rcuisine, -n))) + geom_bar(stat="count")+coord_flip()
...
rownames(tf_mat) <- NULL 
tf_mat %>% arrange(idf) %>% head(10) %>% kable() %>% kable_styling() 
```
