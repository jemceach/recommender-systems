...
# Latent-Factor Collaborative Filtering Recommender with matrix factorization by SVD
# Recommender based on SVD approximation with column-mean imputation.

svdr <- Recommender(data=ratings_train,method='SVD', param=list(k=10, normalize='center'))

# normalizing by subtracting average rating per user;
# note that we don't scale by standard deviations here;
# we are assuming people rate on the same scale but have different biases
    
#Train
svd10_Train_pred <- predict(svdr,ratings_train,type='ratings')
 
# Evaluation of Train Predication 
svd10_Train_Acc <- calcPredictionAccuracy(svd10_Train_pred,ratings_train)

#Test/Evaluate the result from Unknown evaluations for data.
svd10_pred <- predict(svdr,ratings_test_known,type='ratings')
svd10_Test <- calcPredictionAccuracy(svd10_pred,ratings_test_unknown)

#Results
Result <- rbind(SVD_Train_10 = svd10_Train_Acc,
                SVD_Test_10 = svd10_Test)

kable(Result,caption=" Result of SVD Test ") %>% kable_styling()
svd10_Train_Acc
```
...
## calculate the root mean square error on the known ratings
...
fsvd10_test = calcPredictionAccuracy(ratings_test_unknown_predict,  ratings_test_unknown)

Result = rbind(Result, FSVD_Train_10 = fsvd10_train,FSVD_Test_10 = fsvd10_test)

kable(Result,caption=" Result of Funk SVD") %>% kable_styling()
```
