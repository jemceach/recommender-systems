...
user <- data4 %>% select(userID, budget,activity, drink_level, smoker) %>% inner_join(data3,user_profile,by='userID') 
## set factors
...
Algorithm to predict an individuals ratings using decision trees.

### Build GBM Model 

Our GBM model measures the relative influence of our select restaurant and and user characteristics to make user-rating predictions. gbm & resample packages.
...
PatronDataModel <- data %>% select(userID, placeID,rating, budget, price,drink_level,alcohol,smoking_area,smoker, Rcuisine)
...
## Prediction summary
predictions <- as.factor(round(patron_prediction_1))
predictions %>% summary() %>% kable(caption="Predicted Rating Counts",row.names = c(1,2,3), col.names = "n") %>% kable_styling() %>% column_spec(1,bold = T)
```
### Evaluation

RMSE/MSE/MAE/RMSLE evaulation. Turn into loop. Metrics package.

```{r}
#calculate/view RMSE/MSE/MAE/RMSLE
rmse_fit1 <- Metrics::rmse(actual = PatronTest$rating, predicted = patron_prediction_1)
mse_fit1 <- Metrics::mse(actual = PatronTest$rating, predicted = patron_prediction_1)
mae_fit1 <- Metrics::mae(actual = PatronTest$rating, predicted = patron_prediction_1)
rmsle_fit1 <- Metrics::rmsle(actual = PatronTest$rating, predicted = patron_prediction_1)

print(paste0("RMSE: ", rmse_fit1))
print(paste0("MSE: ", mse_fit1))
print(paste0("MAE: ", mae_fit1))
print(paste0("RMSLE: ", rmsle_fit1))

```

#### Confusion Matrix

Evaluation of precision and accuracy. Caret package. 

```{r}
PatronTest$rating<-as.factor(PatronTest$rating)
eval_cm <- confusionMatrix(predictions,PatronTest$rating)
eval_cm$byClass %>% kable() %>% kable_styling()
```
