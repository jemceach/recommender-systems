...
ib_c <- Recommender(getData(evalu, "train"), "IBCF", 
      param=list(normalize = "center",method="Cosine"))
```
...
p_ub <- predict(ub_c, getData(evalu,"known"),type="ratings")
p_ib<- predict(ib_c, getData(evalu,"known"),type="ratings")
...
error <- rbind(
  ub_c = calcPredictionAccuracy(p_ub, getData(evalu, "unknown")),
  ib_c = calcPredictionAccuracy(p_ib, getData(evalu, "unknown"))
)
...
Our next method uses *Singular Value Decomposition* *(SVD)*. We started with 10 latent features, then increased and decreased them, to find better RMSE of our predictor. The latent features were derrived from the factors each user and item have in common within a given user-item matrix. 

We compared the standard SVD approach to Funk SVD, Z-score SVD, and *Alternating Least Squares (ALS)* due to the similiarities between the algorithms. 
...
We can also look at SVD using the funk approach. Funk SVD decomposes a matrix (with missing values) into two components: `U` and `V`. The singular values are folded into these matrices. The approximation for the original matrix can be obtained by `R = UV'

```{r svd-method, warning=T}
# Prep data
...
Our prediction results can be seen below:

```{r svd-results, echo=F}
# STASH RAJ's train calc predictions

### TRAIN PREDICTIONS
### svd10_Train_pred <- predict(svdr,ratings_test_unknown,type='ratings')
### svd10_Train_Acc <- calcPredictionAccuracy(ratings_train,svdr)
### fsvd10_train <- calcPredictionAccuracy(ratings_train,  ratings_train_predict)
### reconstruct the rating matrix as R = UV' & calculate the RMSE on known ratings
...
Our data source was rich with categorical features containing user data such as height, weight, religion, personality-types and more.  Thus, we wanted to also try to incorporate a hybrid approach to look at user and restaurant attributers to build our prediction model using a different machine learning techinique -- Gradient Boosting. 
...
1.  [**Gab41**:])(https://gab41.lab41.org/recommender-systems-its-not-all-about-the-accuracy-562c7dceeaff) It's Not All About the Accuracy
...
6.  [**Medium**:](https://medium.com/@alfonsollanes/how-do-you-measure-and-evaluate-the-quality-of-recommendation-engines-2e91db5952af): How Do You Measure and Evaluate the Quality of Recommendation Engines
7.  [**UC Business Analytics R Programming Guide**:](http://uc-r.github.io/gbm_regression) Gradient Boosting Machines
