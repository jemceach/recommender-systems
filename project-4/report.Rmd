...
restaurant <- data2 %>% select(placeID, name, city,price,franchise, alcohol,smoking_area) %>% mutate(name=tolower(gsub("[\u00ef\u00bf\u00bd\'_']", " ", name))) %>% mutate(city=tolower(city))%>%filter(grepl("slp|san|s\\.l\\.p",city)) %>% select(-city) 
...
user <- user %>% mutate(weight = weight * 2.2) # convert kg to lbs
...
```{r}
ggplot(data, aes(x=rating)) + geom_histogram(bins=3, color='#000000', fill='#D47A7A') +labs(title="Raw Ratings Distribution by Patron Budget") + facet_wrap(data$budget, nrow=1)

ggplot(data, aes(x=rating)) + geom_histogram(bins=3, color='#000000', fill='#D47A7A') +labs(title="Raw Ratings Distribution by Restaurant Pricing") + facet_wrap(data$price, nrow=1)

ggplot(data, aes(x=rating)) + geom_histogram(bins=3, color='#000000', fill='#D47A7A') +labs(title="Raw Ratings Distribution by Activity") + facet_wrap(data$activity, nrow=1)

ggplot(data, aes(x=birth_year)) + geom_histogram(bins=10, color='#000000', fill='#D47A7A') +labs(title="Raw Ratings Distribution by Birth Year") + facet_wrap(data$rating, nrow=1)

ggplot(data, aes(x=weight)) + geom_histogram(bins=10,color='#000000', fill='#D47A7A') +labs(x="weight (lbs)", title="Raw Ratings Distribution by Activity") + facet_wrap(data$rating, nrow=1)
...
#create a real rating matrix
umat <- as(umat,"realRatingMatrix")

# evaluation method using training set of 
evalu <- evaluationScheme(umat, method="split", train=0.9, given=5, goodRating=0)
```
...
[**recommenderlab: A Framework for Developing and
Testing Recommendation Algorithms**:](https://cran.r-project.org/web/packages/recommenderlab/vignettes/recommenderlab.pdf) 
