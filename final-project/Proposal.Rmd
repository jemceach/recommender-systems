...
For our final project, we decided to work with a data subset from the Yelp challenge. Our subset focuses on Yelp reviews from the metropolitan area of Phoeniz, Arizona. Our data takes into account not only reviews, but also businesses, users and check-in's. Our data consists of: 
...
This data is stored within the data folder of our repository for review and replication purposes.

## Recommender 

We aim to build out a system where we recommend businesses to users based on past reviews, check-in's and location. We will continue to builf off our project 4, as we use the same methods to recommend new businesses to users based on past reviews. Our recommendation approaches will include content based and SVD filtering using cosine similarity, collaborative filtering using least squares and location-based filtering. 

If time permits, we may even have a chance to do NLP processing on the individual reviews to better understand sentiment analysis, using a 0 or 1 to classify low and high rankings. We would also like to try implimenting a shiny app for recommendations.
