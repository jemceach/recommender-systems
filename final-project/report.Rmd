...
```{r}
df <-business %>% inner_join(review, by="business_id")
```
...
```{r eval=T}
# Tidy data for spark
user_item_data <- df %>% select(user_id, business_id, stars) %>% transform(u_id=match(user_id, unique(user_id)))%>% transform(i_id=match(business_id, unique(business_id)))

spark_data <- user_item_data %>% select(stars, u_id, i_id) %>% mutate(rating=stars, user=u_id, item=i_id) %>% select(rating, user, item)
...
fit <- ml_als(sprk_df, rating_col = "rating", user_col = "user", item_col = "item")
#summary(fit)
```
