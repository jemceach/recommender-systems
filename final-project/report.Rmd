...
Show example predictions using ml_recommend
...
We noted the user-based Collaborative Filter (UBCF) model gave good result with high recall and same level of Precisions. When compared with all the models for n = 1,3,5,15,20 for top n items, as number of top n increases we  noted Decrese in the Precison and increase in Recall, since Recall sesitive to False Negatives (i.e. do not suggest an Business which is highly likely to be visited), so in contrast we aim for higher Recall with maxium Precision.
